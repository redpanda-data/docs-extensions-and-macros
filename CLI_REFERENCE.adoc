...
* `rpk-docs` - Generate AsciiDoc documentation for rpk CLI commands using JSON tree output. Requires rpk with --print-tree support (v26.2.0+).
...
Generates comprehensive CLI reference documentation for rpk (Redpanda Keeper) using the `rpk --print-tree` JSON output. This modern approach replaces the legacy Docker-based help text parsing with faster, more accurate JSON parsing. Key features: - Parses structured JSON from `rpk --print-tree` (requires rpk v26.2.0+) - Automatically includes rpk plugins (connect, ai, check, etc.) - Tracks plugin versions separately - Supports overrides.json for description improvements - Generates versioned JSON files for downstream consumers (tooltips, etc.) - Generates diffs between versions for release notes

*Why use it:*

The legacy approach spun up Docker, ran help commands, and parsed text output. This was slow, error-prone, and didn't capture all metadata. The new --print-tree flag outputs the entire command tree as structured JSON, including plugins, enabling accurate and efficient documentation generation without Docker dependencies.
...
`-t, --tag <tag>`::
Git tag for released content (e.g., v26.2.0)
...
Override starting version for multi-release processing

`--fetch-binary`::
Download rpk binary for the specified version instead of using local rpk

`--rpk-path <path>`::
Path to custom rpk binary (e.g., /tmp/rpk-dev for testing)
...
`--from-source <path>`::
Path to rpk Go source directory (e.g., ~/redpanda/src/go/rpk) for pre-releases - builds for current platform only

`--linux-from-source <path>`::
Path to rpk Go source directory - builds in Linux Docker container for complete command coverage including Linux-only commands

`--source-ref <ref>`::
Git branch or tag to checkout (e.g., dev, v26.2.0). If no --from-source or --linux-from-source path, sparse-clones from GitHub

`--include-linux`::
Merge with Linux rpk tree from Docker for complete platform coverage (requires --tag with Docker image that supports --print-tree)

`--preserve-from <path>`::
Path to existing docs directory to preserve cloud conditionals and editorial includes from (e.g., ../docs/modules/reference/pages/rpk)
...
[,bash]
----
# Generate docs using local rpk (must support --print-tree)
...
npx doc-tools generate rpk-docs --overrides custom-overrides.json

# Full workflow: document new release
VERSION=$(npx doc-tools get-redpanda-version)
npx doc-tools generate rpk-docs --tag $VERSION --fetch-binary
----

*Requirements:*

- rpk v26.2.0 or later (with --print-tree support)
- Or use --fetch-binary to download a specific version
- For Docker fallback: Docker must be running
