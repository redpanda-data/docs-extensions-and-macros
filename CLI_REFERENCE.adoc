...
A feature must only be documented as enterprise under its approved external name. This check catches new license-gated features in core that have no registry entry yet, registry pointers that no longer match core, and stale connect enterprise lists.
...
`--connect-ref <ref>`::
Connect git ref for info.csv (default: "main")
...
`--antora <path>`::
Local path to rp-connect-docs antora.yml (default: fetch from rp-connect-docs main)

`--properties <path>`::
Local path to a generated cluster-properties JSON; enables existence checks for gating properties

`--skip-connect`::
Skip the connect info.csv comparison
