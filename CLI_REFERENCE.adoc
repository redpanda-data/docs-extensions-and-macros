...
`--date <date>`::
Authored release date as YYYY-MM-DD (confirmed on the PR)

`--body <file>`::
Path to the rpchangelog release body markdown (relative to repo root, must stay inside the repository)
...
`--section-only`::
Print only the candidate section rather than the full page
