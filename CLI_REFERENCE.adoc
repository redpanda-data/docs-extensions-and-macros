...
Checks that every streaming docs version currently published has a Kapa source, so Ask AI can scope answers to it. Exit status is meaningful: 0 every version is covered, 1 a version is missing, 2 the command itself failed (Kapa or the sitemap unreachable, bad credentials). A version counts as covered when Kapa has a source named `Documentation (X)` for it AND that source is assigned to a source group. A source left unassigned is global in Kapa, which means it is returned for EVERY query on every version, so it is reported here as well.
