...
- REDPANDA_GITHUB_TOKEN: A Personal access token (PAT) that has `repo` permissions for the `redpanda-data` GitHub organization.
...
- GLOBAL_ATTRIBUTES_GITHUB_TOKEN: A Personal access token (PAT) that has `repo` permissions for the GitHub organization defined in `org`.
