...
/**
 * @doc-tools validate enterprise-features
 *
 * Checks the enterprise features registry (the shared component's
...
validation
  ...
  .action(async (options) => {
    ...
    try {
      ...
      const coreHeader = await fetchText(`${RAW}/redpanda-data/redpanda/${options.tag}/src/v/features/enterprise_features.h`, true)
      const configurationHeader = await fetchText(`${RAW}/redpanda-data/redpanda/${options.tag}/src/v/config/configuration.h`, true)
      const infoCsv = options.skipConnect
        ? undefined
        : await fetchText(`${RAW}/redpanda-data/connect/${options.connectRef}/internal/plugins/info.csv`, true)
      const disablePage = options.disablePage
        ? readLocal(options.disablePage)
        : await fetchText(`${RAW}/redpanda-data/docs/${options.docsRef}/modules/get-started/pages/licensing/disable-enterprise-features.adoc`, true)
      const antoraYaml = options.antora
        ? readLocal(options.antora)
        : await fetchText(`${RAW}/redpanda-data/rp-connect-docs/main/antora.yml`, true)
