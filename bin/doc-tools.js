...
automation
  ...
  .action((options) => {
    ...
    if (!oldTag) {
      ...
      if (oldTag) {
        console.log(`ℹ️  Using latest-redpanda-tag from Antora attributes for --diff: ${oldTag}`);
      } else {
        console.log('ℹ️  No --diff provided and no latest-redpanda-tag found in Antora attributes. Skipping diff.');
      }
    }
