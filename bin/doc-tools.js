...
automation
  ...
  .action((options) => {
    ...
    const make = (tag, overrides) => {
      console.log(`⏳ Building property docs for ${tag}…`);
      const args = ['build', `TAG=${tag}`];
      if (overrides) {
        args.push(`OVERRIDES=${overrides}`);
      }
      const r = spawnSync('make', args, { cwd, stdio: 'inherit' });
      if (r.error) {
    ...
    if (oldTag) {
      ...
      if (!fs.existsSync(oldDir)) make(oldTag, overridesPath);
    }

    make(newTag, overridesPath);
