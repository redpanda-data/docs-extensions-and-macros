...
=== Integrations
* **algolia-indexer** - Index content for Algolia search (records typed `Doc`, `Lab`, or `Solution`)
* **compute-end-of-life** - Calculate and display EOL information
* **find-related-docs** - Find and link related documentation (deprecated, removed in 6.0; use solutions-catalog)
* **find-related-labs** - Find and link related lab exercises (deprecated, removed in 6.0; use solutions-catalog)

=== Solutions
* **solutions-catalog** - Validate the `solutions` component, derive every `page-solution-*` attribute, compute `page-related-solutions` for Product Docs pages, and publish `assets/data/solutions.json` plus `assets/data/solutions-graph.json`

=== Shared helpers (`extension-utils/`)
* **deployment-type** - `getDeploymentType(attributes)`: the one mapping from `env-kubernetes`/`env-linux`/`env-docker`/`env-cloud`/`page-cloud` to a deployment label, used by every extension above
* **categories** - `createCategoryMap`, `parseCategoryList`, `normalizeCategories`: validate `page-categories` against `valid-categories.yml` and add missing parents
...
=== Link health
* **url-to-xref** - Convert docs.redpanda.com URLs in content into validated xrefs and warn about URLs that match no published page
* **external-link-checker** - Verify external URLs respond and report dead links in the build log
...
No extension required: core Antora (>= 3.1.8) generates redirects for https://docs.antora.org/antora/latest/page/page-aliases/[page aliases] automatically on every build. Define aliases in your `antora.yml`:
