...
A field whose value cannot be sourced truthfully is left out rather than guessed, and so is an empty list, so that every key present is a claim. `verified`, the Doc Detective evidence for a solution, is not emitted for that reason: the specs are generated into a transient run directory at test time and never reach the content catalog, so a build has neither a spec count nor a run time to report. Publishing a manifest alongside the solution is what would make the field emittable, and projecting it is then a change to this emitter alone.
