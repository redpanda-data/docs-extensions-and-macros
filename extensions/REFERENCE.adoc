...
enabled (optional)::
Set to `false` to register the extension without checking anything, so a playbook shared between a scheduled build and a preview build can turn the network requests off. Default: `true`.
