...
min_score (optional)::
Minimum score an edge needs to be shown. Default `0.3`. Two pages that share only a top-level category score `0.1`, so parent-only overlap never shows.
...
Eligible Product Docs pages receive `page-related-solutions`, only when non-empty: `[{id, title, url, description, difficulty, duration, technologies, provenance, score, reason}]` in rank order. Eligible means: a published page (`page.out`) in the latest version of its component, the component is not `solutions`, `home`, `shared`, `search`, `data-platform`, or `self-managed`, the layout or role is not an umbrella layout (`home`, `component-home-v3`, `data-platform`, `labs-home`, `labs-search`, `search`, `404`, `solutions-home`, `solution`, `solution-step`), and the page does not set `page-exclude-related-solutions`.
...
| `0.3` per shared subcategory + `0.1` per shared top-level category, capped at `0.85`
|===
