...
WARNING: Deprecated since 5.43.0 and removed in 6.0. The build logs one warning per run while it is registered. Solution overviews carry their related Product Docs in `page-solution.relatedDocs` (see <<Solutions catalog>>), with provenance and a stable ranking instead of a superset category match.
