...
ignore (optional)::
List of regular expressions matched against URL paths. Matching URLs are left untouched with no warning. Use this for paths on the docs domain that live outside the Antora catalog. Default: `['^/api/']` (the API reference is hosted by Bump.sh).
