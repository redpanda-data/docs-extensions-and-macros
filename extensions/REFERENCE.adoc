...
`{ generatedAt, siteUrl, settings: { maxRelated, minScore }, edges: [{ doc, docUrl, solution, provenance, score, shown, rank, reason }] }`. Every recommendation a reader sees is explained here, and so is every one they do not.
