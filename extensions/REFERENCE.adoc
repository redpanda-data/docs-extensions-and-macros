...
min_score (optional)::
Minimum score an edge needs to be shown. Default `0.6`, which a category-only edge reaches with two shared subcategories (`0.3 + 0.3`); one subcategory plus the parent bonus scores `0.4` and stays hidden. Explicit and approved edges score `1.0` and `0.9` or more, so they always clear it.
...
| `0.3` per shared subcategory + `0.1` per shared top-level category (parents capped at `0.1` per edge in total), capped at `0.85`
|===

Why the threshold sits at two subcategories: an audit of the demo build's edges found that pages sharing one broad subcategory such as `Clients` with a solution were not relevant to it, while pages sharing two or more were. One subcategory is too weak a signal in practice, and a parent bonus says only that the page is in the same product area, so it can never carry an edge over the line on its own. Where a single page is genuinely the right companion, `page-solution-related-docs` (explicit, `1.0`) is the tool; `relationships.yml` (`approved`) covers the cases an editor confirms after the fact.
...
`createCategoryMap(validCategories)` builds `{ categories: Set, subcategories: Set, parentMap: Map }` from the `page-valid-categories` list. `parseCategoryList(value)` splits a comma list. `normalizeCategories(list, map)` returns `{ categories, invalid, parentsAdded }`: authored order kept, parents appended, unknown values removed. `validate-attributes` logs the `invalid` and `parentsAdded` results; `solutions-catalog` treats `invalid` as fatal.
