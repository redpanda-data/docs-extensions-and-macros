...
WARNING: Deprecated since 5.39.0 and removed in 6.0. The build logs one warning per run while it is registered. Solution overviews carry their related Product Docs in `page-solution.relatedDocs` (see <<Solutions catalog>>), with provenance and a stable ranking instead of a superset category match.
...
WARNING: Deprecated since 5.39.0 and removed in 6.0. The build logs one warning per run while it is registered. Product Docs pages now receive `page-related-solutions` from <<Solutions catalog>>, which only shows a solution when the score clears a threshold, so the "Suggested labs" block that appeared on every categorized page goes away with it.
...
Published solutions must also pass the section checks on the converted HTML: the overview needs `h2` headings `Architecture`, `Prerequisites`, and `Production considerations` (matched after lower-casing, collapsing whitespace, and dropping trailing punctuation); every step needs an `h2`/`h3` whose text starts with `Verify` or a block with the `solution-verify` role. Every `link:{attachmentsdir}/...` on any page of the module must point at a file that exists in the module's attachments.
...
| `page-solution-nav`
| `{ home: {title, url}, overview: {title, url}, steps: [{id, title, url, order, duration}] }`.
...
| Scalar mirrors of the overview values, so a step page template can read them without parsing JSON. `page-solution-featured` is `true`/`false`; lists are comma joined.
...
draft::
Unpublished (`page.out` deleted and the URL added to `siteCatalog.unpublishedPages`) unless `include_drafts` or `SOLUTIONS_INCLUDE_DRAFTS=true`. Never in the catalog or the graph when unpublished; when built, it is in the graph with `shown: false` and the build warns that drafts are being built. Section checks are skipped for drafts.
...
| `rejected`
| `relationships.yml` entry with `status: rejected`. Beats every other signal.
...
`{ generatedAt, siteUrl, solutions[], facets: { categories, technologies, difficulty, platforms } }`. `solutions` holds the public record of every published and deprecated solution, sorted by id. Each facet is `[{ value, count }]` sorted by value. The same JSON is set as the `solutions-catalog` attribute on every component version; the UI helper reads `page.component.asciidoc.attributes['solutions-catalog']` the way `get-index-data` reads `generate-index-data` output.
