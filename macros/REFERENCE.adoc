...
This applies to the inline macro. The tables from `enterprise_features` need no marker, because their expiration column already states the licensing behavior as text.

The beta badge needs no special handling either: its label is real text content, so it converts to `(beta)`.
