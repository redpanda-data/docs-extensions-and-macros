...
The macro validates its target against the published property reference: a `redpanda-properties-<tag>.json` attachment in the `reference` module of the build, the same data the tooltips fetch at runtime. See <<prop-version-selection>> for which one a given page uses. To migrate existing plain-backtick property mentions, use `doc-tools generate migrate-property-refs`.
...
. For a component with no property data of its own (`cloud-data-platform`, `connect`), the `streaming` dataset -- that component's own series if its version names one, otherwise the newest -- then `ROOT`, then any. Unversioned doc sets, and those whose version numbers are unrelated to Redpanda releases, correctly track the current release; one versioned per Redpanda release gets that release's properties rather than the newest.

Selecting by series rather than newest tag matters in practice: the docs repo's main branch carries the previous series' JSON alongside its own, and a prerelease tag could land there too. Newest-tag-wins would hand that file to pages of a different release.

When a versioned component publishes property data for other series but none for the page's own, the macro declines to validate rather than borrow a neighbouring release's data. It logs one warning per component version naming the missing file, the tags it did find, and the `doc-tools generate property-docs --tag` command that produces it. `prop:` targets on those pages render unvalidated and their tooltips carry no data, so either generate the series' JSON on that branch or leave `prop:` out of that version's content.
...
When no page in the current component documents the property, what the macro emits depends on whether another component's reference does.

*Published elsewhere but not here.* A topic property on a Cloud page, for example: the Cloud component publishes only the cluster and object storage references. The property does not apply to this audience, so the macro renders plain `<code>` with no link, no `data-property-name` marker, and therefore no tooltip, and logs a warning naming the property and the file. Linking into another component would send Cloud readers to self-managed documentation for a property they cannot set. Either publish the property on that component's reference pages (check the property's include tags) or reword the mention and drop the macro. Set `property-validate` to `off` to silence the warning.

The exception is a component that publishes no property reference at all (`connect`, or a preview site). With no local reference to prefer, it borrows the `streaming` (or `ROOT`) component's pages through a component-qualified xref.

*Published nowhere in the build.* A deprecated property, or one held back by include tags. The marker and its tooltip stay, because the published JSON still describes the property accurately, but there is no page to link: `link=true` renders unlinked and logs a warning. A tooltip-only mention is silent.

When no component in the build publishes property pages at all, the link falls back deterministically to the property's `config_scope` (`cluster`, `broker`, or `topic`).
