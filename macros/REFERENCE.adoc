...
. For a component with no numeric version and no property data of its own (`cloud-data-platform`, `connect`), the newest `streaming` dataset, then `ROOT`, then any. These doc sets track the current release.
