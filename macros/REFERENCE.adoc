...
NOTE: For most property references, prefer the newer <<prop>> macro: it validates names against the published property reference, derives the target page automatically, and powers hover tooltips. `config_ref` remains for cases that need its Kubernetes `storage.tiered.config` prefixing or an explicit path.
