...
link::
Set to `true` to also link the property to its reference page. The page is derived from the property's `config_scope` in the published JSON (`cluster`, `broker`, or `topic`), so writers don't need to know the page. This supersedes the `config_ref` macro's linking for most uses.

page::
Optional reference-page override (module-relative, without `.adoc`), for example `properties/object-storage-properties`.
