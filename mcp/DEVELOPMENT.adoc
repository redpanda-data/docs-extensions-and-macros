...
Developer guide for working on the Redpanda Docs Tools MCP server.
...
│       ├── generate.js            # Generation tools
│       └── version.js             # Version tools
├── prompts/                       # MCP prompts
...
Create a new Markdown file in `prompts/`:

[,bash]
----
prompts/my-docs-guide.md
----
...
Edit `bin/doc-tools-mcp.js` and add your prompt to the prompts array:

[,javascript]
----
{
  name: 'my-docs-guide',
  description: 'Guide for working with [specific documentation]',
  arguments: []
}
----

And add a case to the prompt handler:

[,javascript]
----
case 'my-docs-guide':
  promptText = fs.readFileSync(
    path.join(__dirname, '../prompts/my-docs-guide.md'),
    'utf8'
  );
  break;
----

=== Step 3: Document the prompt

Update `prompts/README.adoc` to document when Claude should use this prompt.

See link:prompts/README.adoc[Prompts documentation] for more details.
