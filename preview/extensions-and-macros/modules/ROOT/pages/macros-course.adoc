...
*Step 1 -- see it fail.* Rebuild. The name matches nothing, so it renders as plain text and the log says so:
...
*Step 2 -- register it.* Open `preview/shared/modules/ROOT/partials/enterprise-features.yml`. Add an entry to the `features` list (indentation matters -- match the entries around it):
...
Rebuild. The mention above now renders as a styled term with a tooltip -- no `status` key at all means GA, so it's available everywhere, on every version, with no badge.
