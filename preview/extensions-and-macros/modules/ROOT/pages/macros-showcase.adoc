...
A bare name like that one stays module-relative within this component's own
`reference` module. Qualify it with a module (`module:page`) or a component
and module (`component:module:page`) to point anywhere else the property is
documented:
...
That target doesn't exist in this sandbox, so it isn't rendered live here --
but the override is never build-verified either way, so a qualified target
behaves exactly like the bare one above: nothing checks that
`cloud:reference:properties/cluster-properties.adoc` actually exists or
documents the property, until a reader clicks it.
...
Redpanda Cloud has no Enterprise Edition license -- customers never apply or
manage one -- so a page in a Cloud component has nothing true to say about a
license, its expiration, or its restrictions. This page is not a Cloud
component, so the `+enterprise:Tiered Storage[]+` call above renders styled, as
usual.

The xref:macro-fixtures-cloud::index.adoc[Cloud fixtures^] page sets
`env-cloud`, so the same `+enterprise:Tiered Storage[]+` call renders as plain
text there, with no styling, tooltip, or link, and
`enterprise_features::redpanda[]` renders a warning instead of a table --
regardless of what the registry says, and even for a feature the registry
does recognize.

This also covers the Cloud rendering of single-sourced self-managed prose:
the check reads the page actually being converted (the Cloud stub page), not
which repo the included `+enterprise:+` mention was written in.

== Where else to look

* xref:macro-fixtures-cloud::index.adoc[Cloud fixtures^] -- only
  `cloud_supported` properties are available, release candidates are
  ignored because Cloud runs released Redpanda, and `+enterprise:+` renders
  plain because Cloud has no Enterprise Edition license.
