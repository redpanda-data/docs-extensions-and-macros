...
The `prop` macro marks a configuration property so the docs UI can decorate it with a hover tooltip. Targets are validated against the published property JSON (an attachment in the streaming component's reference module).

A cluster property: prop:cloud_storage_enabled[] in a sentence.

Linked, page derived from the property's scope: prop:iceberg_enabled[link=true] in a sentence.

An unknown name renders as written but logs a build warning: prop:not_a_real_property[] in a sentence.
