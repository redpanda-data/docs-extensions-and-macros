 ...
 */
function convertDocument (content, convertible) {
  ...
  const converted = lines.map((line) => {
    ...
    const result = convertLine(line, convertible)
    total += result.count
    return result.line
  })
 ...
 */
function migrate ({ docsDir, propertiesJson, write, exclude = [
  ...
  const walk = (dir) => {
    for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
      ...
      if (entry.isDirectory()) {
        ...
        const result = convertDocument(original, convertible)
        if (result.count > 0) {
  ...
  return { files, conversions, changed, ambiguous }
}

module.exports = { classifyNames, convertLine, convertDocument, migrate }
