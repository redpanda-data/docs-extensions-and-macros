...
function runAudit (options) {
  ...
  if (surface === 'properties' && !options.extracted) {
    throw new Error('Missing required option --extracted <path> (the property extractor\'s raw JSON output, without overrides applied)')
  }

  const adapter = adapterFactory()
  return adapter.audit({
    overridesPath: path.resolve(options.overrides),
    extractedPath: options.extracted ? path.resolve(options.extracted) : undefined
  })
}
