...
The docs generator validates properties and reports issues:

[cols="1,2"]
|===
| Error file | Content

| `empty_description.txt`
...
| `max_without_min.txt`
| Properties with maximum but no minimum

| `min_without_max.txt`
| Properties with minimum but no maximum
|===

Fix reported issues by adding overrides in `overrides.json`.
