// Points at the overrides file and its schema, both of which live in the repo a
// writer already has open. The previous pointer was a Confluence page, and
// Confluence is retired, so it sent every writer who read this line to a dead
// end. A repo-relative path cannot rot the same way.
const AUTOGEN_NOTICE = '// This content is autogenerated. Do not edit manually. To change a description, add a link, or scope text to one docs build, edit docs-data/property-overrides.json (fields are documented in docs-data/property-overrides.schema.json and docs-data/README.adoc) and regenerate.\n';
'use strict';
...
/**
 * Which reference page a property is written to.
 *
 * The one place that decides, so a link's target page cannot disagree with the
 * page the property actually lands on. generatePropertyPartials uses it to pick
 * the partial to append to; applyPropertyLinks uses it to tell a same-page
 * reference from a cross-page one.
 *
 * Returns null for a property with no scope, which is not rendered at all.
 *
 * @param {Object} prop - The property object.
 * @returns {('cluster'|'topic'|'broker'|'object-storage'|null)}
 */
function propertyPageFor(prop) {
  if (!prop || !prop.config_scope) return null;
  switch (prop.config_scope) {
    ...
    case 'broker':
      return 'broker';
    case 'cluster':
      return isObjectStorageProperty(prop) ? 'object-storage' : 'cluster';
    case 'object-storage':
...
function generatePropertyPartials(properties, partialsDir, onRender) {
  ...
  Object.entries(properties).forEach(([key, prop]) => {
    ...
    const page = propertyPageFor(prop);
    switch (page) {
      case 'topic':
...
function generateAllDocs(inputFile, outputDir) {
  // bigIntJson, not JSON.parse: the uint64 and int64 limits carried as property
  // maxima are beyond what a JS number holds, so a plain parse rounded them and
  // the generated partials -- and therefore the published reference pages --
  // showed 18446744073709552000 where the data says 18446744073709551615.
  const data = bigIntJson.parse(fs.readFileSync(inputFile, 'utf8'));
  ...
  // partials, the deprecated partial and the topic mappings partial all see the
  // same text.
  //
  // This is what lets a description stay plain prose. Markup mixed into the
  ...
  for (const warning of linkResult.warnings) console.warn(`Warning: ${warning}`);
  if (linkResult.unmatched.length > 0) {
    // A link key that matches nothing is silent link rot: the override looks
    // maintained and the page has no link. Name every one.
    console.warn(
      `Warning: ${linkResult.unmatched.length} declared link(s) matched no text and were not applied:`
    );
    for (const { property, key } of linkResult.unmatched) {
      console.warn(`   ${property}: links["${key}"]`);
    }
  }
