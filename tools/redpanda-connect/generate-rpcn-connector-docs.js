...
//
// The empty meta/body regions are load-bearing. Pages include this file with
// `[tag=meta]` and `[tag=body]`, and Asciidoctor logs `tag 'meta' not found
    ...
    for (const item of items) {
      ...
      if (CONNECTOR_PAGE_TYPE_DIRS.has(partialTypeDir)) {
        ...
        if (descriptionOut.trim()) {
          ...
          // The meta region only sets :description: when the connector has a
...
function backfillPageDescriptions (connectorData, { pagesRoot, dryRun = false } = {}) {
  ...
  for (const [dataKey, items] of Object.entries(connectorData)) {
    ...
    for (const item of items) {
      ...
      // A page migrated to the description partial carries the meta include
      ...
      // flattener the description partial's meta region uses, so a page
